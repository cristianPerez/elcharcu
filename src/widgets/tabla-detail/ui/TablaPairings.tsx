...
interface TablaPairingsProps {
  readonly pairings: readonly TitleDescription[];
  readonly recommendations: readonly string[];
  readonly resultNote: string;
}

/** Maridajes y recomendaciones (fondo verde): tarjetas, checklist y nota final. */
export function TablaPairings({
  pairings,
  recommendations,
  resultNote,
}: TablaPairingsProps): ReactNode {
  return (
      ...
      <Container>
        ...
        <div className="mt-7 flex flex-col gap-4 md:flex-row">
          {pairings.map((pairing) => (
            <PairingCard key={pairing.title} pairing={pairing} />
          ))}
        </div>
